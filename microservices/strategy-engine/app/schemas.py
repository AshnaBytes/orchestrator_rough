...
class StrategyInput(BaseModel):
    ...
    user_offer: float = Field(..., description="The latest price offered by the user.")
    ...
    history: List[Dict[str, Any]] = Field(
        default_factory=list, description="Log of conversation turns"
    )
...
class StrategyOutput(BaseModel):
    ...
    policy_type: str = Field(
        default="rule-based", description="The type of policy that made this decision."
    )
    policy_version: Optional[str] = Field(
        default="1.0.0", description="The version of the policy used."
    )
    decision_metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional data for audit/logging."
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "decision_metadata": {
                    "reason": "User offer > 70% of MAM, applying mid-point formula."
                },
            }
        }
    )
